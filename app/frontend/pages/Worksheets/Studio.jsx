...
export default function Studio({ skill, template, defaults, overridable }) {
  ...
  return (
    <Shell active="Worksheets">
      <div className="n">Worksheet · {template}</div>
      <h1 style={{ fontSize: 30, margin: 'var(--space-2) 0 var(--space-6)' }}>{skill.title}</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: 'var(--space-8)', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--color-neutral-600)', margin: 0 }}>
            Re-printing gives the identical sheet. “New practice” changes the numbers.
          </p>
        </div>
      </div>
    </Shell>
